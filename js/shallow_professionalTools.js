/*
  This function creates menu for the Professional Tools category of the Shallow level.
...
*/
function createShallowProfessionalToolMenu(){
  document.body.innerHTML="";
  createImage("images/background/background.png","bgImg1");
  createHeading1("Shallow- Professional Tools");
  //Button for creating the play scene of Professional Tools category in Shallow level by calling playProfessionalTool() function when clicked
  var playButton= createButton("Play");
  playButton.onclick= playProfessionalTool;

  //Button for displaying information on how to play Professional Tools category in Shallow level by calling shallowFoodchainInfo() function when clicked
  var infoButton=createButton("Information");
  infoButton.onclick= shallowProfessionalToolInfo;
...
/*
  This function creates information page for the Professional Tools category of the Shallow level.
  The page contains information about how to play the category of game and a button for going back to the category menu.
  This function does not accept any parameters and does not have any return values.
*/
function shallowProfessionalToolInfo(){
  document.body.innerHTML="";
  document.body.classList.add("infoBlock");
  var para = document.createElement("P");
  var textNode = document.createTextNode("In this category you have to match a image of a professional person to his/her tool.On clicking a block an image will be revealed and then click another block. If the image underneath the newly clicked block is a match with the previously revealed image then both the images will be unhidden. However, if the image is not a match then the images will be hidden again and you will have to memorise the locations of the images. Game is over when each image is matched with its pair. Unless that happens keep clicking blocks and memorising the locations to match images.");
  para.appendChild(textNode);
  para.setAttribute("align","center");
  document.body.appendChild(para);
  var BackButton= createButton("Back");
  BackButton.onclick= createShallowProfessionalToolMenu;
}
/*
  This function creates the playing scene for the Professional Tools category of the Shallow level.
...
function playProfessionalTool(){
  ...
  img[0].onclick = createChangingImage(0,shallow_professionaltools[0].url);
  img[1].onclick = createChangingImage(1,shallow_professionaltools[1].url);
  img[2].onclick = createChangingImage(2,shallow_professionaltools[2].url);
  img[3].onclick = createChangingImage(3,shallow_professionaltools[3].url);
  img[4].onclick = createChangingImage(4,shallow_professionaltools[4].url);
  img[5].onclick = createChangingImage(5,shallow_professionaltools[5].url);
  img[6].onclick = createChangingImage(6,shallow_professionaltools[6].url);
  img[7].onclick = createChangingImage(7,shallow_professionaltools[7].url);
  img[8].onclick = createChangingImage(8,shallow_professionaltools[8].url);
  img[9].onclick = createChangingImage(9,shallow_professionaltools[9].url);
  img[10].onclick = createChangingImage(10,shallow_professionaltools[10].url);
  img[11].onclick = createChangingImage(11,shallow_professionaltools[11].url);
  /*
  ...
  */
  function changeImage (val, url){
    ...
    if((clickInformation[0] != undefined) && (clickInformation[1] == undefined)) {
      clickInformation[1]= shallow_professionaltools[val].name;
    }
    if((clickInformation[0] == undefined) && (clickInformation[1] == undefined)){
      clickInformation[0]=shallow_professionaltools[val].name;
    }
    ...
    if((clickInformation[0]!= undefined) && (clickInformation[1]!= undefined)){
      ...
      if ( (clickInformation[0]=="books" && clickInformation[1]=="student") || (clickInformation[0]=="student" && clickInformation[1]=="books")){
      ...
      else if ( (clickInformation[0]=="engineeringtool" && clickInformation[1]=="engineer") || (clickInformation[0]=="engineer" && clickInformation[1]=="engineeringtool")){
        score++;
        scoreDivision.removeChild(scoreText);
        scoreText = document.createTextNode("Current Score: "+score);
        scoreDivision.appendChild(scoreText);
      ...
      else if ( (clickInformation[0]=="doctor" && clickInformation[1]=="doctortool") || (clickInformation[0]=="doctortool" && clickInformation[1]=="doctor")){
        score++;
        scoreDivision.removeChild(scoreText);
        scoreText = document.createTextNode("Current Score: "+score);
        scoreDivision.appendChild(scoreText);
      ...
      else if ( (clickInformation[0]=="shovel" && clickInformation[1]=="farmer") || (clickInformation[0]=="farmer" && clickInformation[1]=="shovel")){
        score++;
        scoreDivision.removeChild(scoreText);
        scoreText = document.createTextNode("Current Score: "+score);
        scoreDivision.appendChild(scoreText);
      ...
      else if ( (clickInformation[0]=="plane" && clickInformation[1]=="pilot") || (clickInformation[0]=="pilot" && clickInformation[1]=="plane")){
        score++;
        scoreDivision.removeChild(scoreText);
        scoreText = document.createTextNode("Current Score: "+score);
        scoreDivision.appendChild(scoreText);
      ...
      else if ( (clickInformation[0]=="softwareengineer" && clickInformation[1]=="laptop") || (clickInformation[0]=="laptop" && clickInformation[1]=="softwareengineer")){
        score++;
        scoreDivision.removeChild(scoreText);
        scoreText = document.createTextNode("Current Score: "+score);
        scoreDivision.appendChild(scoreText);
      ...
      //if no match has been detected then changing both the images back to the theme image
          ...
          img[imageDivisionValues[0]].onclick = createChangingImage(imageDivisionValues[0],shallow_professionaltools[imageDivisionValues[0]].url);
          ...
          img[imageDivisionValues[1]].onclick = createChangingImage(imageDivisionValues[1],shallow_professionaltools[imageDivisionValues[1]].url);
